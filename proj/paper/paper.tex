...
refer to as MILP when there is no ambiguity, is appealing because, assuming that the solver is
correct, it will give an optimal solution, however it suffers from a few major
drawbacks.  First and foremost, depending on the input, the runtime time is worst case
exponential in the number of constraints.  Moreover, we found that even
commercial solvers can struggle with large problem sizes, either taking large
amounts of time to produce any feasible solution or simply not terminating in a
reasonable amount of time. For our motivating use case, we would ideally feed
in many labeled queries, each producing tens of constraints, hence exponential
runtime in the number of constraints greatly reduces the usefulness of the
solution. 
...
$$
f(w) = \langle \text{HardTanh}(Aw - \gamma), \overrightarrow{1}\rangle 
$$
...
\begin{algorithmic}[1]
	...
	\While{current time $ < T$}
		...
		\For{$i =  1,...20$}
			...
			\EndFor
			\If{$f(w) < f(w^*)$}
				\State $w^* \gets w$ \Comment{Update best solution}
			\EndIf
		\EndFor
	\EndWhile
	\\\Return $w^*$
